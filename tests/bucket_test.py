...
@pytest.mark.asyncio
async def test__remove_bucket_with_wait(client):
    """Should remove bucket and wait for deletion"""
    bucket = await client.create_bucket("test-bucket-remove")
    await bucket.write("entry-1", b"data", timestamp=1_000_000)

    # Remove and wait - this should complete without error
    await bucket.remove(wait=True, timeout=10.0)

    # Verify bucket is removed
    with pytest.raises(ReductError):
        await client.get_bucket("test-bucket-remove")


@pytest.mark.asyncio
async def test__remove_entry_with_wait(bucket_1):
    """Should remove entry and wait for deletion"""
    # Verify entry exists
    entries = await bucket_1.get_entry_list()
    entry_names = [e.name for e in entries]
    assert "entry-1" in entry_names

    # Remove and wait
    await bucket_1.remove_entry("entry-1", wait=True, timeout=10.0)

    # Verify entry is removed
    entries = await bucket_1.get_entry_list()
    entry_names = [e.name for e in entries]
    assert "entry-1" not in entry_names


@pytest.mark.asyncio
async def test__wait_for_deletion_already_deleted_bucket(client):
    """Should handle waiting for already deleted bucket"""
    bucket = await client.create_bucket("test-bucket-wait")
    await bucket.remove()

    # Wait for deletion should complete quickly (bucket already gone)
    await bucket.wait_for_deletion(timeout=5.0)


@pytest.mark.asyncio
async def test__wait_for_deletion_already_deleted_entry(bucket_1):
    """Should handle waiting for already deleted entry"""
    await bucket_1.remove_entry("entry-1")

    # Wait for deletion should complete quickly (entry already gone)
    await bucket_1.wait_for_deletion("entry-1", timeout=5.0)


@pytest.mark.asyncio
async def test__wait_for_deletion_timeout(bucket_1):
    """Should timeout if deletion takes too long"""
    # This test creates a scenario where we try to wait for a non-deleting resource
    # The wait_for_deletion should raise an error because entry-1 is READY, not DELETING
    with pytest.raises(ReductError, match="not being deleted"):
        await bucket_1.wait_for_deletion("entry-1", timeout=1.0)
